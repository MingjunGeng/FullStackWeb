...
function dogCardMaker(_ref) {
  ...
  heading.textContent = "Breed: ".concat(breed);
  image.src = imageURL;
  image.classList.add('dog-image');
  dogCard.classList.add('dog-card'); // creating the hierarchy

  dogCard.appendChild(image);
  dogCard.appendChild(heading); // adding some interactivity
...
function getDogs(breed, count, selector) {
  _axios.default.get("https://dog.ceo/api/breed/".concat(breed, "/images/random/").concat(count)).then(function (resp) {
    console.log(resp); //console.log(resp.data)
    ...
    for (var i = 0; i < resp.data.message.length; i++) {
      ...
      var doggo = dogCardMaker(dogObj); // entryPoint.appendChild(doggo)
...
_axios.default.get("https://lambda-times-api.herokuapp.com/breeds").then(function (resp) {
