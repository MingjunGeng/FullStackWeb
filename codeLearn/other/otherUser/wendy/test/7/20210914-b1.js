...
const runners = [
  ...
  // ==== Challenge: ====
  // The event director needs both the last name and T-Shirt size of each runner to send to the designer so that the designer can design custumize race shirts for them. Combine both the last names and T-Shirt size and populate a new array called shirtOrder. 
  ...
  runners.forEach(function(element){
    ...
    // console.log(element.shirt_size);
    // newArr.push(element.last_name , element.shirt_size);
    newArr.push(`${element.last_name} , ${element.shirt_size}`);
  });
  console.log(newArr);
...
// const shirtOrderTwo = runners.map(function(item){
//     return `${item.last_name}, ${item.shirt_size}`;
//   });
  
//   console.log(shirtOrderTwo);

//    const  shirtOrderTwo = runners.map(function (item) {
//        return `${item.last_name}, ${item.shirt_size}`;
//    });
//    console.log(shirtOrderTwo);
//    const  shirtOrderTwo = runners.map( (item) => {
//        return `${item.last_name}, ${item.shirt_size}`;
//    });

const  shirtOrderTwo = runners.map( item => `${item.last_name}, ${item.shirt_size}`);
   console.log(shirtOrderTwo);


  // for loop
  
  const shirtOrderThree = [];
  for(let i=0; i< runners.length; i++){
    shirtOrderThree.push(`${runners[i].last_name}, ${runners[i].shirt_size}`);
  }
  console.log(shirtOrderThree);